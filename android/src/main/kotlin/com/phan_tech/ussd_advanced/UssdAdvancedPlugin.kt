...
class UssdAdvancedPlugin: FlutterPlugin, MethodCallHandler, ActivityAware, BasicMessageChannel.MessageHandler<String?>  {
  ...
  override fun onDetachedFromActivity() {
    ...
    // Don't clear context here to persist it between hot reloads
    activity = null
  }
    ...
    if (message != null && event != null) {
      USSDController.send2(message, event!!) {
        ...
        try {
          ...
          Log.e(TAG, "Error in onMessage: ${e.message}")
          reply.reply(null)
        }
      }
    } else {
      Log.e(TAG, "onMessage: message or event is null. Message: $message, Event: $event")
      reply.reply(null)
    }
  }
  ...
  private fun hasPermissions() : Boolean {
    if (context == null) {
      Log.e(TAG, "hasPermissions: Context is null")
      return false
    }
  ...
  private fun requestPermissions() {
    if (context == null || activity == null) {
      Log.e(TAG, "requestPermissions: Context or activity is null")
      return
    }
  ...
  private fun singleSessionUssd(ussdCode:String, subscriptionId:Int) : CompletableFuture<String>? {
    if (context == null) {
      Log.e(TAG, "singleSessionUssd: Context is null")
      return null
    }
    ...
    if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.O){
      ...
      try {
        ...
        val simManager: TelephonyManager = if (subscriptionId > 0) {
        ...
        Log.e(TAG, "Error in sendUssdRequest: ${e.message}")
        res.completeExceptionally(RequestExecutionException(e.message ?: "Unknown error in sendUssdRequest"))
      }
  ...
  private fun multisessionUssd(ussdCode:String, subscriptionId:Int, @NonNull result: Result) {
    if (context == null || activity == null) {
      Log.e(TAG, "multisessionUssd: Context or activity is null")
      result.error("CONTEXT_NULL", "Plugin context or activity is null", null)
      return
    }
    ...
    try {
      // Ensure USSDController has context before proceeding
      if (!::USSDController.isInitialized || !USSDController::context.isInitialized) {
        USSDController.context = context!!
      }
      ...
      ussdApi.callUSSDInvoke(activity!!, ussdCode, slot, object : USSDController.CallbackInvoke {
        override fun responseInvoke(ev: AccessibilityEvent) {
          ...
          try {
            ...
            Log.e(TAG, "Error in responseInvoke: ${e.message}")
        ...
        override fun over(message: String) {
          try {
            ...
            Log.e(TAG, "Error in over callback: ${e.message}")
            result.error("CALLBACK_ERROR", e.message, null)
          }
        }
      })
    } catch (e: Exception) {
      Log.e(TAG, "Exception in multisessionUssd: ${e.message}")
      result.error("MULTISESSION_ERROR", e.message, null)
    }
  }
  ...
  private fun multisessionUssdCancel() {
    if(event != null){
      ...
      Log.e(TAG, "multisessionUssdCancel: event is null")
    }
  }
  ...
  private fun defaultUssdService(ussdCode:String, subscriptionId:Int) {
    if (context == null || activity == null) {
      Log.e(TAG, "defaultUssdService: Context or activity is null")
      return
    }
    ...
    try {
      ...
      if(!_useDefault){
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M){
          ...
          val phoneAccountHandleList = telecomManager.callCapablePhoneAccounts
          if (phoneAccountHandleList != null && phoneAccountHandleList.isNotEmpty() && sim < phoneAccountHandleList.size)
            intent.putExtra("android.telecom.extra.PHONE_ACCOUNT_HANDLE",
              phoneAccountHandleList[sim]
            )
        }
      ...
      Log.e(TAG, "Error in defaultUssdService: ${e.message}")
      throw e
    }
  }
  ...
  private fun isTelephonyEnabled(): Boolean {
    if (context == null) {
      Log.e(TAG, "isTelephonyEnabled: Context is null")
      return false
    }
    ...
    try {
      ...
      Log.e(TAG, "Error checking telephony: ${e.message}")
      return false
    }
  }
}
